...
import { 
  collection, addDoc, deleteDoc, updateDoc, doc, query, where, onSnapshot, orderBy 
...
export default function VoiceTodoApp() {
  /* ---------- STATE ---------- */
  const [user, setUser] = useState(null); // Tracks Login
  const [tasks, setTasks] = useState([]);
  const [transcript, setTranscript] = useState("");
  const [aiResponse, setAiResponse] = useState("");
  const [isProcessing, setIsProcessing] = useState(false);
  ...
  /* ---------- 1. FIREBASE AUTH & DB LISTENER ---------- */
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, (currentUser) => {
      setUser(currentUser);
      
      if (currentUser) {
        // If logged in, sync tasks from Cloud Database
        const q = query(
          collection(db, "tasks"), 
  ...
  const toggleTaskDB = async (taskIndex) => {
    const task = tasksRef.current[taskIndex];
    if (task) await updateDoc(doc(db, "tasks", task.id), { completed: !task.completed });
  };

  const deleteTaskDB = async (taskIndex) => {
    const task = tasksRef.current[taskIndex];
    if (task) await deleteDoc(doc(db, "tasks", task.id));
  };

  const clearTasksDB = async () => {
    const promises = tasksRef.current.map(t => deleteDoc(doc(db, "tasks", t.id)));
    await Promise.all(promises);
  };

  /* ---------- 3. BACKEND CALL (Stable Callback) ---------- */
  const sendCommandToBackend = useCallback(async (text, messageId) => {
    ...
    try {
      ...
      /* 🔥 CHANGED: UPDATE DB INSTEAD OF LOCAL STATE */
      if (data.action === "add" && data.taskText) {
        await addTaskDB(data.taskText);
      }
      else if (data.action === "delete" && Number.isInteger(data.taskIndex)) {
        await deleteTaskDB(data.taskIndex);
      }
      else if (data.action === "complete" && Number.isInteger(data.taskIndex)) {
        await toggleTaskDB(data.taskIndex);
      }
  ...
  const toggleSession = async () => {
    try {
      if (isConnected) {
        ...
        await conversation.startSession({
          agentId: AGENT_ID, // 🔒 Using your existing Agent ID
          // workletPaths removed for simplicity unless you specifically have these files locally
        });
      }
    } catch (err) {
      console.error("Toggle error:", err);
  ...
  if (!user) {
    return (
      <div className="min-h-screen w-full bg-[#0f172a] text-white flex items-center justify-center p-4 selection:bg-indigo-500/30 font-sans relative overflow-hidden">
        {/* Background Blobs */}
        <div className="absolute top-0 left-[-10%] w-[500px] h-[500px] bg-indigo-600/20 rounded-full blur-[100px] pointer-events-none" />
        <div className="absolute bottom-0 right-[-10%] w-[500px] h-[500px] bg-purple-600/10 rounded-full blur-[100px] pointer-events-none" />
        
        <div className="w-full max-w-md bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl p-8 shadow-2xl text-center z-10">
          <div className="w-20 h-20 bg-indigo-500/20 rounded-2xl flex items-center justify-center mx-auto mb-6 ring-1 ring-white/10">
             <Zap size={40} className="text-indigo-400" />
          </div>
          <h1 className="text-3xl font-bold mb-2">VoiceTask AI</h1>
          <p className="text-slate-400 mb-8">Sign in to sync your tasks across devices.</p>
          <button 
             onClick={handleLogin}
             className="w-full bg-white text-slate-900 font-bold py-4 rounded-xl hover:bg-indigo-50 transition-all flex items-center justify-center gap-3 shadow-lg hover:shadow-indigo-500/20"
           >
             <User size={20} />
             Sign in with Google
           </button>
        ...
        <div className="w-full max-w-lg lg:max-w-5xl bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl shadow-black/50 ring-1 ring-white/5 overflow-hidden flex flex-col">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className={`flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 rounded-full border backdrop-blur-md transition-all duration-500 ${
               ...
               <button 
                ...
                className="p-2 bg-white/5 hover:bg-red-500/20 hover:text-red-400 rounded-full transition-colors border border-white/5"
                title="Logout"
              >
                <LogOut size={16} />
              </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 lg:divide-x divide-white/5">
            ...
            <div className="p-6 md:p-10 flex flex-col justify-center min-h-[400px]">
              ...
              <AnimatePresence>
                {error && (
                  <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="bg-red-500/10 border border-red-500/20 rounded-xl p-3 mb-6 flex items-center gap-3">
                    <AlertCircle className="text-red-400" size={18} />
                    <p className="text-red-200 text-xs">{error}</p>
                  </motion.div>
                )}
              ...
              <div className="flex flex-col items-center justify-center flex-1">
                <div className="relative group">
                  {(isConnected || isSpeaking) && (
                    <>
                      <motion.div className="absolute inset-0 bg-indigo-500/20 rounded-full" animate={{ scale: [1, 1.6], opacity: [0.6, 0] }} transition={{ duration: 2, repeat: Infinity, ease: "easeOut" }} />
                      <motion.div className="absolute inset-0 bg-indigo-500/20 rounded-full" animate={{ scale: [1, 1.4], opacity: [0.6, 0] }} transition={{ duration: 2, repeat: Infinity, ease: "easeOut", delay: 1 }} />
                    </>
                  )}
              ...
              <div className="mt-8 space-y-3 min-h-[120px] flex flex-col justify-end">
                <AnimatePresence mode="wait">
                  {!transcript && !aiResponse && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center text-slate-500/40 text-sm italic py-4">
                      Your conversation history will appear here...
                    </motion.div>
                  )}
                  ...
                  {transcript && (
                    <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="self-end max-w-[90%]">
                      <div className="bg-indigo-600 text-white px-4 py-2.5 rounded-2xl rounded-tr-sm text-sm md:text-base shadow-lg">{transcript}</div>
                      <p className="text-[10px] text-slate-400 text-right mt-1 mr-1">You</p>
                    </motion.div>
                  )}
